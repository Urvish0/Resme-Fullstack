...
def is_rate_limited(client_id: str) -> bool:
    ...
    try:
        ...
        if request_count > RATE_LIMIT:
            logger.warning(
                f"[RATE_LIMIT] Exceeded for client {client_id}: {request_count}/{RATE_LIMIT}"
            )
            return True
        else:
            logger.info(
                f"[RATE_LIMIT] Allowed for client {client_id}: {request_count}/{RATE_LIMIT}"
            )
            return False
        ...
        logger.warning(
            "[RATE_LIMIT] Redis error, failing open",
